...
describe("verifyLoadableUnderPi", () => {
	...
	// Vehicle moved to its own packages (vehicle-core/vehicle-server/vehicle-client/
	// vehicle-client-pi) -- see each package's own pi-load-harness.test.ts for its
	// equivalent load-safety coverage; vehicle-client-pi's is the one that matters
	// most, since it's the only Vehicle package actually loaded as a Pi extension.
