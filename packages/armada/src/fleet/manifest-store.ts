...
import { type ArmadaManifest, decodeArmadaManifest, MAX_MANIFEST_BYTES, type VehicleSpec } from "./manifest.js";
...
export async function upsertManifestVehicle(path: string, vehicleJson: string): Promise<ManifestMutationOutcome> {
	...
	try {
		...
		return {
			ok: false,
			diagnostics: [diagnostic("MANIFEST_JSON_INVALID", "error", "/vehicle", error instanceof Error ? error.message : String(error))],
		};
	}
