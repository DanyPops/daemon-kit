...
import { executeCleanup, type ObservedProcess, planDuplicateCleanup } from "../src/index.js";
...
describe("duplicate cleanup", () => {
	...
	it("requires exact approval and refuses changed process state without signaling", async () => {
		...
		const stale = await executeCleanup({
			...
			currentProcesses: () =>
				Promise.resolve([...processes, { pid: 44, executable: "/opt/papyrus/cli.js", command: "/opt/papyrus/cli.js serve" }]),
			managedPid: 42,
			handle: undefined,
			vehicle: vehicle(),
			terminate,
