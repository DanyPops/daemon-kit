 ...
 * implementation removes by construction. Duplicated from
 * @danypops/vehicle-server's own version.ts (identical, ~20 lines, zero
 * dependencies) rather than depended on -- a client-side consumer
 * (connectWithVersionCheck) has no other reason to need vehicle-server at all.
 */
