...
describe("decodeArmadaManifest", () => {
	it("decodes, brands, sorts, hashes, and freezes a valid manifest", () => {
		const outcome = decodeArmadaManifest(manifestJson([
			...
			{ ...JSON.parse(manifestJson()).vehicles[0], name: "lector", executable: "C:\\Tools\\lector.exe", handlePath: "C:\\Temp\\lector.json" },
		]));
		expect(outcome.ok).toBe(true);
		if (!outcome.ok) return;
		expect(outcome.manifest.vehicles.map((item) => item.name)).toEqual(["lector", "papyrus"]);
	...
	it.each([
		...
		["relative executable", manifestJson([{ ...JSON.parse(manifestJson()).vehicles[0], executable: "bin/papyrus" }]), "MANIFEST_PATH_NOT_ABSOLUTE"],
		["secret", manifestJson([{ ...JSON.parse(manifestJson()).vehicles[0], arguments: ["--token=raw-secret"] }]), "MANIFEST_SECRET_MATERIAL"],
