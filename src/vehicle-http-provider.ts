 ...
 * its Vehicle operations to a RemoteVehicleClient (vehicle-http-client.ts,
 * part of the compiled `vehicle` export) over the same Bearer-authenticated
 * loopback transport every other daemon-kit daemon already uses (http.ts).
 *
 * Daemon-side only: raw TypeScript, like daemon.ts/http.ts, not part of the
 * Pi-loaded compiled surface. Three routes:
...
import { errorResponse, jsonResponse, requireBearerToken, UNAUTHORIZED_RESPONSE } from "./http.js";
import type { VehicleFailure, VehicleFailureCategory } from "./vehicle-errors.js";
import { VehicleError } from "./vehicle-errors.js";
import type { VehicleInvocationOptions, VehiclePrincipal } from "./vehicle-contract.js";
import type { VehicleRegistry } from "./vehicle-runtime.js";
