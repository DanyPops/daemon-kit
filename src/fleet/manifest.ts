...
const SECRET_MATERIAL = /(?:-----BEGIN [A-Z ]*PRIVATE KEY-----|\b(?:api[_-]?key|access[_-]?token|refresh[_-]?token|token|password|secret)\s*[:=]\s*\S+)/i;
...
export function decodeArmadaManifest(text: string): ManifestDecodeOutcome {
	...
	for (const [index, vehicle] of raw.vehicles.entries()) {
		if (names.has(vehicle.name)) diagnostics.push(diagnostic("MANIFEST_VEHICLE_DUPLICATE", "error", `/vehicles/${index}/name`, vehicle.name));
