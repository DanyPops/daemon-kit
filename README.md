...
| `@danypops/vehicle-server` | `vehicle-core`, `daemon-kit` | `VehicleRegistry` (execution engine) at `.`; its authenticated HTTP hosting surface at `./http`. |
...
| `@danypops/vehicle-client-pi` | `vehicle-core`, `daemon-kit` | Projects any `VehicleClient` into native Pi tools, with live availability curation. |
...
very first `registerVehicleTools()` call via `@danypops/daemon-kit`'s
`pi-tool-availability`. `refreshVehicleToolAvailability()` re-fetches the
manifest on whatever cadence the caller chooses (a maintenance-task interval, a
push notification, a session_start recheck) and re-syncs active/inactive state
for known tools, registering any genuinely new operation for the first time.
...
`refreshVehicleToolAvailability()`; `packages/daemon-kit/test/pi-tool-availability.test.ts`
